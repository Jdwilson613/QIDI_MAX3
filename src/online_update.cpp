...
#define UNKNOWN_MODEL -2
#define CONFIG_FILE_MISSING -3
...
void btn_online_update_click_handler()
{
	...
	mksini_free();
    // base_download_url = "http://www." + sys_set.server + ".qidi3dprinter.com:5050/downloads/QD_Max3";
    base_download_url = "https://download_" + sys_set.server + ".qidi3dprinter.com/QD_Max3";
	// 与UI线程分离
    std::thread([]() {
...
static void execute_version_check_event() {
    ...
    if(sys_set.server == "aws")
        url = "https://api.qidi3dprinter.com/code/x_max3_version_info";
        // url = "https://download_aws.qidi3dprinter.com/downloads/QD_Max3/V4.3.15/update_info.ini";
    else
        url = "https://api2.qidi3dprinter.com/code/x_max3_version_info";
        // url = "https://download_aliyun.qidi3dprinter.com/downloads/QD_Max3/V4.3.15/update_info.ini";
                        ...
                        if (j["firmware"].empty()) {
                            ...
                            if (j["firmware"].find("SOC") != j["firmware"].end()
                                ...
                                if (version != local_soc_version) 
                                {
                                    ...
                                    // send_cmd_txt(tty_fd, "t_it", j["firmware"]["SOC"]["description"]["IT"]);
                                    // send_cmd_txt(tty_fd, "t_sp", j["firmware"]["SOC"]["description"]["ES"]);
                                    // send_cmd_txt(tty_fd, "t_kr", j["firmware"]["SOC"]["description"]["KR"]);
                                    // send_cmd_txt(tty_fd, "t_pr", j["firmware"]["SOC"]["description"]["PT"]);
                                    // send_cmd_txt(tty_fd, "t_ar", j["firmware"]["SOC"]["description"]["AE"]);
                                    // send_cmd_txt(tty_fd, "t_hb", j["firmware"]["SOC"]["description"]["HB"]);
...
{
	// 下载更新
    std::string soc_url = base_download_url + "/QD_Max_SOC";
	std::string ui_url  = base_download_url + "/QD_Max3_UI5.0";
