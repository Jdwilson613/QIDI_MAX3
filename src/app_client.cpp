    ...
    if (curl_status == SUCCESS)
    {
        try
        {
            ...
            if (status == 0) // 请求成功 ? 实际上不成功返回的也是0 要根据message判断
            {
                ...
                // B
                mksini_load();
                user_token = mksini_getstring("app", "token", ""); //初始化全局变量user_token
                mksini_free();
...
// TODO_UI: 在二维码界面新建线程每隔90秒刷新并生成二维码 失败时应用图片或者文字展示错误(网络连接失败)
int AppClient::refresh_bind_code(std::string& bind_code)
{
    ...
    if (curl_status == SUCCESS)
    {
        try
        {
            ...
            if (status == 0) // 请求成功 ? 实际上不成功返回的也是0 要根据message判断
            {
                // status 为pending的状态下只会返回http200 status0
...
// TODO_UI: 二维码界面建立线程每90秒调用刷新 退出界面同时也退出线程
int AppClient::update_qrcode()
{
    int curl_status;
    ...
    {
        std::string bind_code;
        int curl_status = app_cli.refresh_bind_code(bind_code);
        if (curl_status == SUCCESS)
        {
            ...
            generate_qrcode(data, APP_QRCODE_PATH, 256); // 设置二维码图片大小为 256x256
...
void *open_bind_thread(void *arg) 
{
    std::thread([]() 
    {
        ...
        while (true) 
        {
            std::string status = (user_token == "") ? "unbound" : "bound"; //打印机自身的状态由token值决定，打印机有三种状态有token值为绑定状态，没token值的时候为解绑状态以及is_in_qr_page=true时为匹配状态
            std::string in_page = is_in_qr_page || is_in_user_page ? "true" : "false"; //接口不能传bool值这里用这种方式替换为string
            if(!first_time_update)
                // TODO: 关闭QIDI Link 服务的原因所在
                if(app_cli.update_device() == SUCCESS)
                ...
                http.timeout_connect(5)
                    ...
                    .on_complete([&](std::string body, unsigned) {
                        ...
                        try {
                            ...
                            if(response["data"].find("token") != response["data"].end()){//这里判断返回信息中是否包含token，如果包含token则设置自身为绑定状态
                                    system("systemctl start QIDILink-client");
                                    std::string rev_user_token = response["data"]["token"];
                                    std::string rev_username = response["data"]["username"];
                                    std::string rev_avatar = response["data"]["avatar"] != nullptr ? response["data"]["avatar"] : "";
                                    std::cout << "receive token = "<< rev_user_token << std::endl;
                                    std::cout << "receive username = "<< rev_username << std::endl;
                                    std::cout << "receive avatar = "<< rev_avatar << std::endl;
                                    std::cout << "status changed"<< std::endl;
                                    user_token = rev_user_token;
                                    MKSLOG_GREEN("now rev_user_token: %s", rev_user_token.c_str());
                                    mksini_load();
                                    mksini_set("app", "token", user_token);
                                    mksini_set("app", "username", rev_username);
                                    mksini_set("app", "avatar", rev_avatar);
                                    mksini_save();
                                    mksini_free();
                                    page_to(TJC_PAGE_QIDI_LINK_LOGIN_SUCCESS);
                                    send_cmd_txt(tty_fd,"t1", rev_username);
                                    ...
                                    if(bind_code != rev_bind_code){
                                        ...
                                        "&code=" + bind_code +
                                        "&SN="+ SN;
                                        generate_qrcode(data, APP_QRCODE_PATH, 368); // 设置二维码图片大小为 368x368
                                        MKSLOG("QR code saved to %s", APP_QRCODE_PATH);
                                        go_to_showqr();
                                    }
            ...
            {
                if (i % 10 == 0){
                    std::cout << "Sleeping for "<< i + 1 <<" second..." << std::endl;
                }
                std::this_thread::sleep_for(std::chrono::seconds(1));
            }
        }
